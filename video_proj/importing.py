...
def yuv_image(b_arr, Nx, Ny, yuv_mode='420', nbytes=-1):
    """
...
def yuv_sequence(v_root, Nx, Ny, yuv_mode='420', t_range=()):
    v_files = glob(v_root)
    if t_range:
        v_files = v_files[t_range[0]:t_range[1]]
    vid = np.empty((len(v_files), Ny, Nx, 3), np.uint8)
    for t,f in enumerate(v_files):
        vid[t] = yuv_image(f, Nx, Ny, yuv_mode=yuv_mode)
    return vid

def y4m_sequence(v_file, t_range=()):
    ...
    for i in t_range:
        ...
        vid[i] = yuv_image(v_file, Nx, Ny, yuv_mode=yuv_mode, nbytes=f_bytes)
    v_file.close()
